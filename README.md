...
فایل کانفیگ JSON به‌صورت کامل قابل تغییر است. برنامه از قالب زیر استفاده می‌کند و آدرس سرور (IP) و در صورت نیاز پورت SOCKS را برای هر تست جایگزین می‌کند.

مثال کامل‌تر (باید مقدارهای `PLACEHOLDER_IP` و `PLACEHOLDER_PORT` را جایگزین کند):
...
در فایل کانفیگ باید جای IP با متن `PLACEHOLDER_IP` مشخص شود. برای جلوگیری از تداخل هنگام اجرای موازی، پورت SOCKS را با `PLACEHOLDER_PORT` بگذارید تا اسکریپت برای هر تست یک پورت جدا اختصاص دهد. باقی مقادیر (UUID، دامنه‌ی TLS و ...) را خودتان تنظیم کنید.

همچنین پیشنهاد می‌شود در کانفیگ، یک inbound از نوع `socks` روی `127.0.0.1` داشته باشید و پورت آن را `PLACEHOLDER_PORT` بگذارید. در این حالت `--proxy` را به شکل `socks5h://127.0.0.1:{port}` تنظیم کنید تا هر ترد از پورت جدا استفاده کند و کانفلیکت ایجاد نشود. اگر قصد دارید پورت ثابت باشد، مقدار پورت را در کانفیگ عدد ثابت بگذارید و `--proxy` را هم با همان پورت ثابت تنظیم کنید.
...
* `--download-url-list` : مسیر فایل حاوی لیست URL دانلود (هر خط یک URL، تست‌ها به صورت Round Robin انجام می‌شوند)
...
* `-p`/`--proxy` : قالب پراکسی محلی (پیش‌فرض `socks5h://127.0.0.1:{port}`)
...
اگر چند URL برای تست دانلود دارید، از `--download-url-list` استفاده کنید تا برای هر تست دانلود به صورت Round Robin بین URLها جابجا شود.
...
* یک inbound از نوع **SOCKS** روی `127.0.0.1` تعریف کنید و پورت آن را `PLACEHOLDER_PORT` بگذارید.
* جای IP سرور را با `PLACEHOLDER_IP` بگذارید تا اسکریپت آن را جایگزین کند.
* `--proxy` را به شکل `socks5h://127.0.0.1:{port}` بگذارید تا پورت انتخابی در پراکسی هم اعمال شود.
...
نمونه اجرای دانلود با لیست URLهای هتزنر:
...
python3 scanner.py \
  ...
  -t 20 \
  -d \
  -D https://speed.hetzner.de/100MB.bin
```

یا با فایل لیست آماده:
...
python3 scanner.py \
  ...
  -t 20 \
  -d \
  --download-url-list hetzner_download_urls.txt
```
...
2. Xray اجرا می‌شود و پراکسی محلی روی یک پورت آزاد روی `127.0.0.1` بالا می‌آید.
...
* **سرعت همیشه صفر است** → بررسی کنید Xray اجرا شده و پراکسی `socks5://127.0.0.1:{port}` فعال است.
