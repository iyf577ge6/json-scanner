...
### ۲) آماده‌سازی فایل تست دانلود

برای اینکه نتایج دقیق و پایدار باشند بهتر است یک فایل تست روی سرور خودتان داشته باشید:

1. یک فایل با حجم مشخص (مثلا 20 تا 100 مگابایت) روی سرور آپلود کنید.
2. URL مستقیم فایل را یادداشت کنید (مثل `https://example.com/test.bin`).
3. در اجرا، این URL را با فلگ `--download-url` به اسکنر بدهید.

> نکته: اگر فایل تست را روی سرور خودتان میزبانی کنید، احتمال محدودیت سرعت کمتر است و خروجی پایدارتر می‌شود.
...
python3 scanner.py \
  ...
  -d \
  -s 500 \
  -D https://example.com/test.bin
```

توضیح مهم پارامترها:

* `-d` تست دانلود را فعال می‌کند.
* `-D` آدرس فایل تست دانلود است.
...
3. با `curl` یک درخواست دانلود از **طریق پراکسی** به URL تعیین‌شده زده می‌شود.
